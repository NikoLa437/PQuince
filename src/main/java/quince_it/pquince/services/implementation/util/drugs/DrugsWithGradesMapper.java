...
public class DrugsWithGradesMapper {
	public static IdentifiableDTO<DrugsWithGradesDTO> MapDrugInstancePersistenceToDrugInstanceIdentifiableDTO(DrugInstance drug, double grade){
		if(drug == null) throw new IllegalArgumentException();
				
		return new IdentifiableDTO<DrugsWithGradesDTO>(drug.getId(), new DrugsWithGradesDTO(drug.getName(), drug.getCode(), drug.getDrugInstanceName(),
