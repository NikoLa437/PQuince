...
public class WorkTimeMapper {
	...
	public static IdentifiableDTO<WorkTimeDTO> MapWorkTimePersistenceToWorkTimeIdentifiableDTO(WorkTime workTime){
		...
		return new IdentifiableDTO<WorkTimeDTO>(workTime.getId(), new WorkTimeDTO(workTime.getPharmacy().getId(),workTime.getStaff().getId(), workTime.getStartDate(), workTime.getEndDate(), workTime.getStartTime(),workTime.getEndTime(),workTime.getPharmacy().getName()));
	}
