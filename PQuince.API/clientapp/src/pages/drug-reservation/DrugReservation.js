...
class DrugReservation extends Component {
    ...
    handleReservation = (amount, date) => {
        ...
        let reservationDTO = {drugId : this.state.drugId, pharmacyId: this.state.pharmacyId, drugAmount: amount, drugPrice: this.state.drugPrice, endDate:date};
        Axios
        .post(BASE_URL + "/api/drug/reserve", reservationDTO).then((res) =>{
    ...
    handlePharmacyClick = (pharmacyId, price, count) => {
        this.setState({reservationModalShow:true, maxDrugAmount: count,drugPrice:price, pharmacyId: pharmacyId });
    }
    ...
    handleDrugSelect = (drug) => {
        ...
        .get(BASE_URL + "/api/pharmacy/find-by-drug/" + drug.Id).then((res) =>{
            this.setState({pharmacies : res.data, drugsPageHidden:true, drugName: drug.EntityDTO.name, drugManufacturer: drug.EntityDTO.manufacturer.EntityDTO.name,
                            drugQuantity: drug.EntityDTO.quantity, drugId : drug.Id });
            console.log(res.data);
        }).catch((err) => {console.log(err);});
    }
    ...
    render() { 
        return (
            <React.Fragment>
                ...
                <ModalDialog show={this.state.openModal} href="/" onCloseModal={this.handleModalSuccessClose} header="Successfully reserved" text="Your reservation is accepted. Further details are sent to your email address."/>
            </React.Fragment>
          );
    }
}
