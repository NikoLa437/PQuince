...
class AllergensModal extends Component {
	...
	componentDidMount() {
		Axios.get(BASE_URL + "/api/allergens", { headers: { Authorization: getAuthHeader() } })
			.then((res) => {
	...
	doesUserContains = (allergenId) => {
		...
		for (let allergen of this.props.userAllergens) {
			if (allergen.Id === allergenId) return true;
		}
		return false;
	};
						...
						<tbody>
							{this.state.allAllergens.map((allergen) => (
								<tr id={allergen.Id} key={allergen.Id}>
									...
									<td>
										{this.doesUserContains(allergen.Id) === true ? (
											<Button onClick={() => this.props.onAllergenRemove(allergen)} variant="danger" style={{ width: "100%" }}>
												Remove
											</Button>
										) : (
											<Button onClick={() => this.props.onAllergenAdd(allergen)} style={{ width: "100%" }} variant="primary">
												Add
											</Button>
										)}
									</td>
